...
import { Edit, Download, Copy, Check } from 'lucide-react'
...
export default function WriteArticle() {
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `article-${Date.now()}.txt`;
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write an article on the topic ${input} in ${selectedLength.text} `
          ...
          {articleLength.map((item, index) => (
            <span
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text
                ? 'bg-blue-50 text-blue-700 hover:text'
